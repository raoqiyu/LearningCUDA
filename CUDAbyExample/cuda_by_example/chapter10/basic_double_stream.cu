...
__global__ void kernel(int *a, int *b, int *c){
	...
	if(idx < N){
		int idx1 = (idx+1)%256;
		int idx2 = (idx+2)%256;
		float as = (a[idx] + a[idx1] + a[idx2]);
		float bs = (b[idx] + b[idx1] + b[idx2]);
		c[idx] = (as + bs)/2;
	}
}
...
int main( void ) {
	...
	if (! prop.deviceOverlap){
		printf("Device will not handle overlaps, so no need up form streams");
		return 0;
	}
	...
	for(int i=0; i<FULL_DATA_SIZE; i+= N*2){
		...
		HANDLE_ERROR( cudaMemcpyAsync( host_c+i, dev_c0,
										...
										stream0));
		HANDLE_ERROR( cudaMemcpyAsync( host_c+i, dev_c1,
										N*sizeof(int),
										cudaMemcpyDeviceToHost,
										stream1));
	}
